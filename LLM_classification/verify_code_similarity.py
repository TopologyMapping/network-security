...
def receive_arguments():
    ...
    parser = argparse.ArgumentParser(description="This code verifies the similarity between Openvas files using LLM.")
    parser.add_argument("--input", required=False, help="Input JSON file with NVTs categories.")
    parser.add_argument("--ip_port", required=True, help="LLM ip and port.")
    parser.add_argument("--number_of_files_compared", type=int, required=False, help="Number of files to be compared. Default=500.")

    return parser.parse_args()

def select_cve_to_analyze(all_cves_selected: list, cves_unique_files, info_op_nvts: dict, key: str):
    """
...
def compare_files_and_store_results(info_op_nvts, category_to_compare: str, number_of_files_compared: int, llm):
    """
    ...
    * The resutls are stored inside the folder 'results' in the root of the project. 
    ...
    while classified < number_of_files_compared:
        ...
        cve_selected = select_cve_to_analyze(all_cves_selected, cves_unique_files, info_op_nvts, category_to_compare)        
        
        unique_file_selected, file_to_compare = select_random_files_to_analyze(cve_selected, info_op_nvts, category_to_compare)
        ...
        if answer != 'yes' and answer != 'no':
            results['errors'].append(f'Unique file: {unique_file_selected}, {category_to_compare} file: {file_to_compare}, Answer: {answer}, Explanation: {explanation}')
            continue 
    ...
    with open(f'./results/verify_simil_openvas__similars_{category_to_compare}.json', 'w') as json_file:
...
if __name__ == '__main__':
    ...
    number_of_files_compared = int(args.number_of_files_compared) if args.number_of_files_compared else DEFAULT_NUMBER_OF_FILES_COMPARED

    compare_files_and_store_results(info_op_nvts, COMPARE_FILES_MAYBE_SIMILARS, number_of_files_compared, llm)
    
    compare_files_and_store_results(info_op_nvts, COMPARE_FILES_SIMILARS, number_of_files_compared, llm)
