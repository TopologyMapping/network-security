    ...
    for scan in data:
        ...
        if uniqueIps.__contains__(ip):
            if ("operating_system" in scan) and ("cpe" in scan["operating_system"]):
                cpe = scan["operating_system"]["cpe"]
                uniqueIps[ip].add(cpe)
        else: 
            uniqueIps[ip] = set()
            if ("operating_system" in scan) and ("cpe" in scan["operating_system"]):
                cpe = scan["operating_system"]["cpe"]
...
uniqueIps = dict(sorted(uniqueIps.items()))

# write info collected in a file
with open('../results/modules_and_ports_censys.txt', 'w') as file:
