...
def wait_for_directory(directory_path=DIRECTORY_PATH, interval=1):
    directory_path = Path(directory_path)

    print(f"Aguardando o diretório ou arquivos dentro de: {directory_path}")
    while not (directory_path.exists() and any(directory_path.iterdir())):
        time.sleep(interval)

    if directory_path.exists():
        if any(directory_path.iterdir()):
            print(f"Arquivos encontrados no diretório: {directory_path}")
            return
        else:
            print(f"Diretório encontrado, mas está vazio: {directory_path}")
            return
